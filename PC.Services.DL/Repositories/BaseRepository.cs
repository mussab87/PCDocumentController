...
namespace PC.Services.DL.Repositories
{
    public class BaseRepository<T> : IBaseRepository<T> where T : class
    {
        ...
        //{
        //    return _context.Set<T>().Where(i => i.Id).Skip(skip).Take(take).ToList();// _context.Set<T>().Find(id);
        //}
        ...
        public async Task<IEnumerable<T>> FindAllIncludeAsync(string[] includes = null)
        {
            IQueryable<T> query = _context.Set<T>();
